...
- **Анимация** — до 100 кадров и 4 с, 25 fps (40 мс — ровно 4 сотых: у GIF задержка в сотых); у анимированной
  картинки — её собственные задержки, кадр, выходящий за 4 с, урезается до их конца (не ниже 20 мс), одиночный кадр
  даёт статичный GIF.
...
- **Фолбэк** (`frameSinkMain.ts`) — тот же кодировщик на главном потоке, поток уступается странице после каждого кадра.
  ...
  сообщение `error` — ошибка конвертации: кадры не кодируются второй раз на главном потоке. Основной путь — Worker:
  он снимает кодирование с главного потока, и страница amo не подвисает на время конвертации. Фолбэк в скрытой
  вкладке медленнее: браузер тормозит `setTimeout`, которым он уступает поток, а через 5 минут в фоне Chrome включает
...
1. **Issue.** Задача заводится в репозитории до правок — чтобы у PR был предмет, с которым аудит сверяет результат.
   ...
   Перед началом работы issue назначается на себя — и свежезаведённая, и взятая из уже открытых: исполнитель виден в
   списке issue, и задачу не возьмут вторым заходом.
...
6. **Повторный аудит и резолв.** Тот же `review-staged` по обновлённой ветке. Резолвится только тред, правка
   ...
   останется.
7. **Архив спеки.** Задача, которая шла через OpenSpec, архивируется до мержа, в той же ветке: `/opsx:archive
   <change>` переносит дельту в `openspec/specs` и change — в `openspec/changes/archive/`, коммит пушится в PR.
   Так требования и код попадают в `master` одним коммитом. Архив после мержа потребовал бы отдельной ветки, PR и
   ещё одного подъёма версии.
8. **Мерж** — после архива: squash, чтобы в `master` от задачи остался один коммит, и с удалением ветки — issue
   закроется сама по `Closes` из тела PR.
