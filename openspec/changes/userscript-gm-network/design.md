...
- [Worker кодирования GIF из blob URL в изолированном мире менеджера может не стартовать из-за CSP мира] →
  ...
  работает; фолбэк — не регрессия функциональности. Проверено (0.7.0, Tampermonkey в Chrome, живой amo): Worker
  стартует, `console.warn` о фолбэке нет; `typeof GM_xmlhttpRequest` и `typeof GM_getValue` в консоли страницы —
  `undefined`.
...
- [`onprogress` в каком-то менеджере не приходит] → третья проверка на `onload` не даёт отдать ядру больше лимита.
  Трафик при этом докачивается, но в Tampermonkey — единственном официально поддерживаемом менеджере — `onprogress`
  приходит. Violentmonkey и другие менеджеры с тем же API работают без гарантий и на живом amo не проверялись.
