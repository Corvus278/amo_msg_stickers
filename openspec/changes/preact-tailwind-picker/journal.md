...
### F1: варианты классов через cva

Пользователь: switch с литералами классов в Button.tsx (getVariantClass) неприемлем — варианты через cva. Решение пользователя: новая runtime-зависимость class-variance-authority, охват — все компоненты с классами по варианту/состоянию.
Критерий F1:
- class-variance-authority в dependencies; Button и все компоненты src/core/ui/Picker, где классы выбираются по варианту или состоянию (switch/тернарники/clsx-объекты по пропу: Tab selected, FeedChip active, StickerCell/MasonryCell busy, StatusBar и т.п.), описывают их через `cva` с `variants`/`defaultVariants`/`compoundVariants`; тип пропа варианта — `VariantProps<typeof …>`, а не ручной union;
- clsx остаётся только для склейки без вариантов;
- вычисленные классы и отрисовка не меняются: собранный CSS и классы элементов на стенде совпадают с HEAD 177e983 во всех вкладках в обеих темах;
- правило в .claude/rules (react.md или code-style.md): «варианты классов — через cva», с примером; design.md — строка про clsx/tailwind-merge дополнена cva, CLAUDE.md «Стек» — cva;
- рост бандла записать; гейт pnpm lint && pnpm test.
Старт — после итогового аудита (он читает тот же код в общем дереве).

### Итоговый аудит, круг 1

Вердикт: critical — кэш object URL (useObjectUrls.ts:13-45, usePickerState.ts:32) отзывается только при удалении: пикер смонтирован всё время страницы, после закрытия живут URL всех показанных с загрузки стикеров (замер: закрыт на «Настройках» — 30 URL / 403 КБ при 3 нужных). Против критерия пользователя про ОЗУ. Было так же в picker.ts.
F2 (критерий): при закрытии пикера отзываются URL, не нужные текущему представлению и обложкам вкладок, либо все; на стенде (6+12+12 стикеров, 5 недавних) закрыт на «Настройках» — живых URL ≤ 3; после повторного открытия всё отрисовано, без видимого мигания обложек; 20 циклов открыть-переключить-закрыть — число живых URL после закрытия не растёт. Решение координатора: выбор «все» или «кроме нужных» — за исполнителем по критерию, приоритет — простота.
Остальное по памяти пройдено: 40 циклов — URL, DOM-узлы, listeners стабильны; таймеры и устаревшие ответы снимаются.
Вопрос пользователю: useTelegramImport пишет в state после ухода с вкладки (нужно по спеке telegram-import — по завершении открыть пак) — исключение из критерия.
Долг: лента GIF в скрытом DOM до следующего открытия (по критерию ок); GIPHY 429 — сырой JSON в статусе; picker.element держит отсоединённую кнопку, если amo удалил composer при закрытом пикере (было и раньше).
Вне change: спека custom-stickers 256 vs 512; CORS api.telegram.org/file в userscript.
Порядок: F2, затем F1 (cva).

### Ответы пользователя по итоговому аудиту

useTelegramImport пишет в state после ухода с вкладки до конца импорта — пользователь принял как исключение из критерия памяти (так требует спека telegram-import, утечки нет).
Вне change заведены issues: #2 CORS api.telegram.org/file в userscript, #3 спека custom-stickers 256 vs 512, #4 сырой JSON в ошибках HTTP. В PR — ссылки.
F2 сделан: при закрытии отзываются все object URL (useObjectUrls(isOpen)); замер исполнителя — 0 живых после закрытия, 20 циклов без роста, мигания нет. Перепроверку ведёт итоговый аудитор.

### F2

F2 (critical итогового аудита): useObjectUrls(isOpen) отзывает весь кэш при закрытии и размонтировании; isOpen протянут createPicker → PickerProvider → usePickerState. Выбор «все», а не «кроме нужных» — простота, представления перечитывают данные при открытии.
...
Гейт: pnpm lint && pnpm test — ok; openspec validate --strict — ok (skip_specs).
Покрытие: сценарии openspec/specs/* пройдены вручную на стенде в 6.3 против старого пикера на реальных API в обеих темах; итоговый аудит — ok после F2; память: 0 живых object URL после закрытия, 20–40 циклов без роста.
Закрыто 25/27. Открыты: 6.4 (живой amo, расширение и userscript) — пользователь; 6.5 (PR, inline-аудит, резолв, мерж после ок пользователя) — координатор.
Доработки: F2 (object URL при закрытии), F1 (варианты через cva, clsx удалён).
Принято пользователем: токены amo целиком (D2); сброс GIF-запроса при смене вкладки; рост бандла +34 КБ сырых / ~+11 КБ gzip (+0,9 КБ от cva); нет пустой шахматной полосы в «Добавить»; фоновый Telegram-импорт пишет в state до конца.
Вне change: issues #2 (CORS Telegram в userscript), #3 (спека 256 vs 512), #4 (сырой JSON в ошибках HTTP).
Правки артефактов для /opsx:update: tasks.md 2.1 — clsx → class-variance-authority; design.md:62 — «источник правды — tailwind-конфиг amo web» раскрывает внутренний источник.
