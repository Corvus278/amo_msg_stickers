...
- Состояния — вариантами по атрибутам: `aria-selected:` у вкладок, `aria-pressed:` у чипов, `disabled:` у кнопок,
  ...
  (вид кнопки, занятая ячейка, drop-зона при перетаскивании, ошибка в строке статуса, открытая панель), описываются
  `cva` (`class-variance-authority`, ~1 КБ): варианты компонента в одной декларации, тип пропа — из `VariantProps`.
  Склейка без вариантов, если понадобится, — `cx` из того же пакета: отдельный `clsx` не нужен.
