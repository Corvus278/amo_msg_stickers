...
- `scripts/version.ts` — чистые функции без рантайм-импортов (типы — в `version.types.ts`, `import type` Node стирает): разбор `MAJOR.MINOR.PATCH` (невалидная строка — ошибка),
...
тега. Цена — при почти одновременных мержах релизы могут появиться не в порядке версий. Поэтому релиз создаётся с
`--latest=false`, а следующий шаг ставит «Latest» (на него ведёт README) наибольшей по `sort -V` версии среди
опубликованных релизов (`gh release list` без черновиков и пре-релизов; тег без релиза «Latest» не получит): последний
завершившийся прогон видит все релизы и ставит верный.
