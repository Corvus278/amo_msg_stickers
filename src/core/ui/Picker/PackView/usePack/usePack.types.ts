...
export type PackState = {
  ...
  /**
   * Удаляет стикер из библиотеки и отзывает его object URL. Промис не отклоняется: ошибка
   * уходит в статус.
   */
  removeSticker: (stickerId: string) => Promise<void>;

  /**
   * Удаляет пак со всеми стикерами и открывает недавние. Промис не отклоняется: ошибка
   * уходит в статус, пак остаётся открытым.
   */
