...
#
#   .ts .tsx      → eslint + tsc --noEmit по проекту
#   .js .mjs .cjs → eslint
#
# Автофикс намеренно не применяется: хук только сообщает, переписывать файл
# за спиной агента он не должен.
#
# Коды возврата:
...
collect_from_command() {
  ...
  while IFS= read -r candidate; do
    ...
    printf '%s\n' "$rel"
  done < <(grep -oE '[A-Za-z0-9_@./~-]+\.(ts|tsx|js|jsx|mjs|cjs)' <<<"$command")
}
...
# tsc --noEmit -p tsconfig.json c инкрементальным кэшем. Бинарь — нативный
# TS 7 из `typescript-native`: `typescript` в зависимостях — версия 6 для
# typescript-eslint, а `node_modules/.bin/tsc` занят кем-то одним из них.
...
run_tsc() {
  local bin="node_modules/typescript-native/bin/tsc"
